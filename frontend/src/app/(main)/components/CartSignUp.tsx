...
export const CartSignUp = () => {
  ...
  let totalPrice = 0;
  data?.map((value: any) => {
    totalPrice += value?.total + value.price;
  });

  const increase = (id: any) => {
    const newData: any = data?.map((value: any) => {
      if (value._id === id) {
        value.quantity += 1;
    ...
    localStorage.setItem("foods", JSON.stringify(newData));

    setData(newData);
  };

  const dicrease = (id: any) => {
    const newData: any = data.map((value: any) => {
      if (value._id === id) {
        if (value.quantity !== 1) {
          value.quantity -= 1;
    ...
    <div className="flex gap-3 w-fit h-fit">
      ...
      <DialogMenu>
        ...
        <DialogContentMenu className="bg-[#404040]">
          <DialogTitleMenu className="flex justify-start items-center gap-3 text-white text-[20px] pb-6 font-semibold">
          ...
          <Tabs defaultValue="Cart" className="w-fit">
            ...
            <TabsContent className="flex flex-col gap-6 pt-6" value="Cart">
              <div className="flex flex-col bg-white gap-6 rounded-[20px] w-full text-start">
                <div className="flex flex-col p-4 bg-white gap-6 rounded-[20px] overflow-scroll size-fit text-start">
                  ...
                  {!data ? (
                    <div className="gap-2 flex flex-col w-[513px] items-center justify-center">
                      <svg
                      ...
                      <h1 className="text-[#71717A] text-[12px]">
                        Hungry? 🍔 Add some delicious dishes to your cart and
                        satisfy your cravings!
                      </h1>
                    </div>
                  ) : (
                    <div className="flex overflow-scroll gap-6 max-h-[440px] h-fit flex-col">
                      {data &&
                        data?.map((value: any, index: number) => (
                          <OrderedFood
                            key={index}
                            increase={() => increase(value._id)}
                            dicrease={() => dicrease(value._id)}
                  ...
                  <button
                    ...
                    //   handleOnClick();
                    // }}
                    className={`border-[#EF4444] border-1 bg-white text-[#EF4444] active:border-[#E4E4E7] rounded-full min-w-fit py-2 px-3`}
                  >
                    Add Food
                  </button>
                </div>
              </div>
              <div className="flex flex-col p-4 bg-white gap-6 rounded-[20px] w-full">
                <h1 className="text-[20px] font-[600]">Payment info</h1>
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between">
                    <h6 className="text-[16px] font-[400] text-[#71717A] w-[220px]">
                      items
                    </h6>
                    <p className="text-[16px] font-[700] text-end w-[220px]">{`${totalPrice} ₮`}</p>
                  </div>
                  <div className="flex justify-between">
                    <h6 className="text-[16px] font-[400] text-[#71717A] w-[220px]">
                      Shipping
                    </h6>
                    <p className="text-[16px] font-[700] text-end w-[220px]">{`5000 ₮`}</p>
                  </div>
                </div>
                <Separator />
                <div className="flex justify-between">
                  ...
                  <p className="text-[16px] font-[700] text-end w-[220px]">{`${
                    totalPrice + 5000
                  } ₮`}</p>
                </div>
                <button
                  ...
                  //   handleOnClick();
                  // }}
                  className={`bg-[#EF4444] text-white active:bg-white active:text-[#EF4444] rounded-full min-w-fit py-2 px-3`}
                >
                  Checkout
                </button>
              </div>
            </TabsContent>
            <TabsContent className="flex flex-col gap-6 pt-6" value="Order">
              <div className="flex flex-col bg-white gap-6 rounded-[20px] w-full text-start">
                <div className="flex flex-col p-4 bg-white gap-6 rounded-[20px] overflow-scroll size-fit text-start">
                  <h1 className="text-[20px] font-semibold">My Cart</h1>
                  {!data ? (
